...
// Автоматически прописываем пространство имен для использования класса Facades\DB
...
// --->> HTTP Requests. Cookies. Куки в Ларавель (https://laravel.com/docs/8.x/requests#cookies). Работа с куки похожа на работу с сессиями. Куки предназначены для хранения пользовательских данных. Сессии хранятся на сервере, Куки хранятся на клиенте. С точки зрения безопасности: все куки, которые создает Ларавель они шифруются и подписываются специальным кодом - если вдруг клиент откроет куки и что-то поменяет, в таком случае они станут недействительными. Куки достаточно безопасны для использования в Ларавель. Работать с куки мы можем используя метод cookie в классе Illuminate\Http\Request или Illuminate\Support\Facades\Cookie
        // Установим свою cookie. При установке куки ничего не должно выводиться (если мы попытаемся установить куки и что-то вывести параллельно, у нас ничего не получится с куки, тк куки отправляются с заголовками к клиенту в браузер
        // Устанавливаем куки используя класс Cookie и метод queue. На вход передаем параметры (название куки, ее значение, срок, на который мы ее устанавливаем (минут)). Возможно установить дополнительные параметры (для какого адреса $path, домена $domain итд: 'name', 'value', $minutes, $path, $domain, $secure, $httpOnly) (https://laravel.com/docs/7.x/requests#input-trimming-and-normalization, https://www.php.net/manual/ru/function.setcookie.php)

//        Cookie::queue ('test', 'Test cookie', 1); // Установим свою cookie. При установке куки НИЧЕГО!!! не должно выводиться вначале страницы (если мы попытаемся установить куки и что-то вывести параллельно, у нас ничего не получится с куки, тк куки отправляются с заголовками к клиенту в браузер
        // Для того, чтобы вывести куки, мы можем использовать класс \Facades\Cookie или использовать объект $request
//        dump (Cookie::get ('test')); // класс \Facades\Cookie
//        dump ($request->cookie ('test')); // объект $request
        // Для удаления куки используется \Facades\Cookie метод queue и метод forget
//        Cookie::queue (Cookie::forget ('test'));


// ---->> HTTP Session. Работа с сессиями (https://laravel.com/docs/8.x/session). Конфигурация сессии находится в файле config/session.php

    // Попробуем что-то записать в сессию (одиночное значение или массив)
       /* $request->session ()->put ('test', 'Test value');*/ // Запись в сессию одиночного значения с помощью метода put (указываем ключ и значение, которое нам будет доступно по этому ключу)
        /*session (['cart' => [
            ['id' => 1, 'title' => 'Product 1'],
            ['id' => 2, 'title' => 'Product 2'],
        ]]);*/ // Записать в сессию многомерного массива

    // Попробуем получить данные из сессии. Для этого мы можем использовать метод get или мы можем обратиться к соответствующей функции хелпер session
        // Получим одиночные данные из сессии
       /* dump (session ('test'));*/
        // Получим многомерный массив (содержимое корзины) из сессии
        /*dump (session ('cart')[1]['title']); // Используем хелпер session
        dump ($request->session ()->get ('cart')[0]['title']);*/ // Используем объект $request вызываем у него метод session и метод get с указанием ключей нужной записи

    // Как добавить в сессию значение уже существующего элемента не удаляя старое, а добавляя новое значение? Для этого существует метод push, который добавит новое значение
        /*$request->session ()->push ('cart', ['id' => 3, 'title' => 'Product 3']);*/ // Указываем ключ, в который мы хотим что-то добавить и указываем массив, который мы хотим туда дописать

    // Удаление данных из сессионного массива. Есть два варианта: 1) Мы можем прочитать значение и сразу же его удалить, 2) Мы можем его сразу удалить
        // 1) Метод pull. Мы можем прочитать значение и сразу же его удалить
        /*dump ($request->session ()->pull ('test'));*/
        // 2) Метод forget. Мы можем его сразу удалить
        /*$request->session ()->forget ('test');*/
    // -->> Метод flush. Мы можем очистить все содержимое сессии
        /*$request->session ()->flush ();*/


        // С сессиями мы можем работать двумя способами:
        // 1. Мы можем работать через объект $request
        /*dump ($request->session ()->all ());*/ // Используем объект $request вызываем у него метод session. Для просмотра всей информации в сессии, используем метод all

        // 2. Использование хелпера session
//        dump (session ()->all ()); // Используем хелпер session. Для просмотра всей информации в сессии, используем метод all



// ---->>> Cache. (https://laravel.com/docs/8.x/cache). Настроить кэширование мы можем в файле конфигурации config/cache.php. Мы сами можем определить, где будет храниться файл настроек кэша (по умолчанию это config/cache.php). Сам кэш находится у нас в storage/framework/cache/data. Если мы выберем место хранения в БД, тогда мы должны создать соответствующую миграцию и создать соответствующую таблицу(https://laravel.com/docs/8.x/cache#prerequisites-database)
        // Кэш нам нужен для того, чтобы ускорить работу нашего приложения, уменьшить нагрузку на сервер (обработка данных и запросы к БД). Как это работает? Один раз достав данные из БД мы кладем их в кэш, и при следующем таком же запросе мы достаем их из кэша. Проверим, если данные есть в кэше - мы берем их из кэша. Если данных в кэше нет - мы берем данные путем запроса к БД. Итд...

        // Для записи элементов в кэш мы используем метод PUT \Facade\Cache
//        Cache::put ('key', 'Value', 60); // На вход передаем данные: название-ключ 'key', по которому нам будут доступны данные, значение 'Value', время, насколько мы кладем данные в кэш (секунды). Мы можем установить время действия кэша или установить его бессрочно (не устанавливаем время - передаем только ключ и значение - тогда данные будут записаны бессрочно), пока мы сами не удалим кэш. Так же, мы можем это сделать с помощью метода forever (Cache::forever('key', 'value');)

        // Для получения элементов из кэша мы используем метод GET \Facade\Cache
//        dump (Cache::get ('key'));

    // Помещаем данные в кэш. Как посмотреть, как происходит кэширование нашего SQL запроса?
        // Для начала, мы проверим, есть ли у нас что-то в кэше используя метод has:
        /*if ( Cache::has ( 'posts' ) ) { // Если у нас есть данные в кэше по ключу 'posts', тогда мы в переменную $posts запишем эти данные
            $posts = Cache::get ('posts');
        } else { // Если в кэше пусто, тогда:
            $posts = Post::orderBy('id', 'DESC')->get (); // Мы получаем эти данные из БД
            Cache::put ('posts', $posts); // Кэшируем полученные данные бессрочно
        }*/

    // Удаляем данные из кэша. Есть несколько способов это сделать:
        // 1) Как и в случае с сессиями, мы можем взять данные из кэша и после этого удалить сам кэш.
//        Cache::put ('key', 'Value', 300); // Кладем данные в кэш
//        dump (Cache::pull ('key')); // Получим данные и удалим их из кэша
//        dump (Cache::get ('key')); // Получаем данные из кэша

        // 2) Для удаления из кэша используем метод forget
//        Cache::put ('key', 'Value', 300); // Кладем данные в кэш
//        Cache::forget ('key'); // Удалим данные из кэша
//        dump (Cache::get ('key')); // Получаем данные из кэша

        // 3) Для удаления из кэша используем метод flush (полное очищение кэша - удаление всех данных из кэша)
//        Cache::flush ();

        $posts = Post::orderBy('id', 'DESC')->paginate (3); // Для того, чтобы получить пагинацию мы должны использовать метод pagination. Для того, чтобы вызвать визуальное оформление пагинации, мы будем использовать метод links в шаблоне конкретной страницы
        /*$posts = Post::orderBy('id', 'DESC')->simplePaginate(3);*/ // Метод simplePaginate используется тогда, когда нам нужно показать пагинацию в формате "« Previous / Next »"
// Если мы хотим самостоятельно создать шаблон для оформления отображения пагинации на странице (Customizing The Pagination View https://laravel.com/docs/8.x/pagination#customizing-the-pagination-view), мы должны в методе links передать путь к нашему шаблону пагинации и данный шаблон будет использован для показа пагинации {{ $paginator->links('views._name_') }}.
// Для кастомизации пагинации нам нужно знать о методах, которые предлагает Ларавель (Paginator/LengthAwarePaginator Instance Methods https://laravel.com/docs/8.x/pagination#paginator-instance-methods) и (Cursor Paginator Instance Methods  https://laravel.com/docs/8.x/pagination#cursor-paginator-instance-methods )
// Вместо того, чтобы создавать свой шаблон, мы можем использовать дефолтные шаблоны с помощью команды (php artisan vendor:publish --tag=laravel-pagination). После выполнения команды они окажутся в папке resources/views/vendor/pagination
...
// ---->> Cоздание формы для заполнения и сохранения данных

    public function create () { // Метод для создания формы для заполнения данными

        $title = 'Create Post';
        $rubrics = Rubric::pluck('title', 'id')->all(); // Получаем рубрики из таблицы rubrics из БД используя модель Rubric метод pluck (получение массива записей, где ключами будут 'id' рубрики, а значениями ее названия 'title'. В данном случае, мы получаем все данные колонки 'title' - используя метод ->all()
        return view('create', compact('title', 'rubrics'));


    }

    public function store (Request $request) { // Метод для сохранения данных из формы отправленных методом POST. На вход внедряем объект запроса (Request $request)

        // Нам нужно как-то поймать данные отправленные из формы. Для этого у нас есть внедренная зависимость объект запроса $request к которой мы можем обращаться.
        // Как получить конкретное поле из формы? Для этого, нам нужно знать его название
        /*dump ($request->input ('title')); // Используем для вывода объект запроса $request и метод input с указанием ключа поля для вывода 'title'
        // Так же, мы можем распечатать другие поля
        dump ($request->input ('content'));
        dd ($request->input ('rubric_id'));*/

        // Таким образом, мы можем получить по отдельности каждое поле и мы можем создать нужный нам объект (в нашем случае это объект модели Post) и заполнить его аттрибуты ('title' равен ____, 'content' равен ____, 'rubric_id' равен ____) и затем вызвать метод save.
        // Продолжение -->



// --->> Validation | Валидация данных. Объявим набор правил валидации полученных из формы методом POST данных
        // Вызываем метод validate. Первым аргументом передаем объект запроса $request, а вторым аргументом - набор валидационных правил в виде ассоциативного массива
        $this->validate ($request, [
            'title' => 'required|min:5|max:100',
            'content' => 'required',
            'rubric_id' => 'integer',
        ]); // Если валидация не будет пройдена будет выброшено исключение и пользователь будет отправлен обратно к форме, где будут показаны ошибки валидации (https://laravel.com/docs/8.x/validation#quick-displaying-the-validation-errors)


/*// ----->> Использование класса validator и его метода make для кастомизации и перевода вывода ошибок валидации, в который мы можем передать свой массив messages (сообщения об ошибках валидации)
        // Создадим массив правил валидации rules
        $rules = [
            'title' => 'required|min:5|max:100',
            'content' => 'required',
            'rubric_id' => 'integer',
        ];
        // Создадим массив сообщений об ошибках валидации messages
        $messages = [
        ...
        $validator = Validator::make ($request->all (), $rules, $messages)->validate (); // Объявляем переменную $validator и записываем в нее: используем класс Validator и метод make (1 аргумент - массив данных, которые нужно валидировать, 2 аргумент - массив правил валидации, 3 аргумент - массив сообщений об ошибках валидации). Затем мы вызываем метод validate ()*/


// ----->> Использование файла локализации для кастомизации и перевода вывода ошибок валидации (расположение resources/lang). По аналогии с EN, мы создаем папку с RU локализацией. Эта локализация должна быть выбрана в настройках config/app.php ('locale' => 'ru',). В папку RU мы копируем файл validation.php из папки EN. После этого, мы находим в файле интересующие нас записи об ошибках и переводим их на русский язык. Но выражение об ошибке будет переведено не полностью. Чтобы перевести сообщение полностью, нужно использовать в этом файле массивы 'custom' (Пользовательские языковые строки проверки) или 'attributes' (Пользовательские атрибуты проверки)


        // <-- Продолжение
// --->> Есть для этого более удобный (проще, быстрее) вариант с использованием метода create и в этот метод мы можем запихнуть всю нашу форму целиком. Вся наша форма целиком доступна через объект request, у которого есть метод all
//        dd ($request->all ());
        Post::create($request->all ()); // Сохраняем нашу форму. Обращаемся к модели Post, вызываем метод create и запихнем всю форму целиком $request->all (). В данном случае, мы используем массовое присваивание. В таком случае, нужно использовать "белый список" (указываем поля, какие мы разрешаем заполнять массово)




    // --->> Flash Data (переменные) - для кратковременных сообщений о каких-то состояниях. Иногда нам нужно сохранить переменную или сообщение в сессии только для следующего запроса (не на весь сеанс работы, а только для следующего запроса). Например, мы что-то отправляем формой, делаем редирект на следующую страницу (это есть следующий запрос) и здесь нам нужно показать эту переменную из сессии и сразу же удалить.

    // В данном методе store, если мы прошли валидацию и сохранили нашу форму в БД, мы можем что-то записать в сессию:
        $request->session ()->flash ('success', 'Данные сохранены!'); // В методе flash передаем ключ 'success' и текст сообщения 'Данные сохранены'. Теперь по маршруту route ('home') у нас будет доступно один раз сообщение ('success', 'Данные сохранены') до следующей перезагрузки страницы.
        ...
        return redirect ()->route ('home'); // Делаем редирект на маршрут 'home' (главная страница)
